...
typedef struct
{
    ...
    SNAIL_CLIENT_CONTEXT_S context_array[SNAIL_CONTEXT_NUM_MAX];
    INT32 count;
    INT32 free_count;
...
INT32 snail_InitClientManager(INT32 count)
{
    ...
    INT32 j = 0;
    SNAIL_CLIENT_CONTEXT_S* context = NULL;
    ...
    for (j = 0; j < SNAIL_CONTEXT_NUM_MAX; ++j)
    {
        ...
        context->hwnd = -1;
        context->taskId = -1;
        context->task_status = SNAIL_TASK_STATUS_UNKNOWN;
    }
...
INT32 snail_AllocClientContext(INT32 hwnd)
{
    ...
    context->hwnd = hwnd;
    context->task_status = SNAIL_TASK_STATUS_START_PENDING;
...
void snail_PutClientContext(INT32 hwnd)
{
    ...
    for (j = 0; j < SNAIL_CONTEXT_NUM_MAX; ++j)
    {
        ...
        if (hwnd == context->hwnd)
        {
            if (SNAIL_TASK_STATUS_RUNNING == context->task_status)
            {
                context->task_status = SNAIL_TASK_STATUS_STOP_PENDING;
            }
            break;
        }
    }
...
INT32 snail_GetClientContextStatus(INT32 hwnd, SNAIL_TASK_STATUS_E* pTaskStatus)
{
    ...
    INT32 j = 0;
    SNAIL_CLIENT_CONTEXT_S* context = NULL;
    SNAIL_TASK_STATUS_E status = SNAIL_TASK_STATUS_UNKNOWN;
    INT32 flag = 0;
