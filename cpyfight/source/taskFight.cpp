...
static int dofight(Idmsoft* dmobj, void* ctxt)
{
    ...
    if (strPosXY != CString(""))
    {
        ...
        if (spcnt >= 2)
        {
            ...
            api_log_MsgDebug("获取:当前:坐标:%ld:%ld", posX, posY);
        }
    }
    ...
    for (;;)
    {
        ...
        if (SNAIL_ERRNO_REALIVE == code)
        {
            api_log_MsgError("定点:攻击:死亡:任务:结束");
        ...
        if (walkTimeDelta > 20.0)
        {
            if (strPosXY != CString(""))
            {
                ...
                {
                    api_log_MsgDebug("归位:死亡:复活:任务:结束");
...
int api_TaskFightInit(Idmsoft* dmobj, void* ctxt)
{
    ...
    //code = dmobj->BindWindow(hwnd, "dx", "dx", "dx", 0);//锁定鼠标
    code = dmobj->BindWindow(hwnd, "dx", "dx2", "dx", 0);//不锁定鼠标
    if (1 != code)
    {
    ...
    if (1 != code)
    {
        api_log_MsgError("窗口:鼠标:锁定:解除:失败, hwnd:%ld", hwnd);
        return ERRNO_BINDWND;
    }

    api_log_MsgDebug("窗口:鼠标:锁定:解除:成功, hwnd:%ld", hwnd);
