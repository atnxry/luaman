...
#define FONTSIGNLECPY1_DIR "font\\exsignle\\"
#define BMPSIGNLECPY1_DIR "bmp\\exsignle\\"

/*
1 从进副本后，对完话开始。

2  寻路。449:383|442:383|433:383|424:383|416:383|408:383|400,383,在这里打怪，一边U一边打（按1到8），一边找提示，白色的 门已开启(字库:font-exs-npc0110.txt)。
   找到提示就结束。找不到提示，打20秒就结束。
  
3  寻路。387:383|381:383|372:384|364:384|357:384|350:384|340:384|332:384|323:382|310:383,在这里打怪，一边U一边打（按1到8），一边找提示，白色的 门已开启。
	找到提示就结束。找不到提示，打30秒就结束。
  
4  寻路。306:375|306:367|304:352|303:346|300:340|295:335|291:328|291:321|290:313|291:305|293:293,  在这里打怪，一边U一边打（按1到8），
	一边找提示，绿色的 隐藏之房显现 。找到提示就结束。找不到提示，打70秒就结束。

5  寻路。291:285|289:278|287:271|285:265|286:258|286:251|286:242|285:234|284:227,  在这里打怪，一边U一边打（按1到8），固定打40秒。

6  寻路。286:217|285:208|280:199|271:196|265:201|262:209,  在这里打怪，一边U一边打（按1到8），一边找提示，白色的 门已开启。找到提示就结束。找不到提示，打50秒就结束。

7  寻路。260:215|259:223|255:230|248:237|242:240|235:241|228:241|220:241|212:239|205:235|195:232|188:225,  在这里打怪，一边U一边打（按1到8），固定打40秒。

8  寻路。180:228|174:228|164:227|161:219|160:207|167:198|178:196,  在这里打怪，一边U一边打（按1到8），一边找提示，白色的 门已开启。找到提示就结束。找不到提示，打50秒就结束。

9  按N对话，点绿色的 进入里面。再点绿色的 我要进去。（从之前的抄）

10 寻路，寻路。189:192|197:187|199:179|199:172|199:162|199:155|199:148  在这里打怪，不按U （按1到8），一边找提示，黄色的 等待时间。找到提示就结束。

11 结束了再打10秒。一边U一边打（按1到8）。然后按空格60下。

12 寻路 199:99|200:93|200:89.接回原来的流程。（按9，再对话出去那里）。
*/
//副本4涉及到字库
struct font_info_t fi_pri[] =
{
    {"门已开启",        FONTSIGNLECPY1_DIR"font-exs-npc0110.txt", "ffffff-101010"},
    {"隐藏之房",        FONTSIGNLECPY1_DIR"font-exs-npc0110.txt", "00ff00-101010"},
    {"等待时间",        FONTSIGNLECPY1_DIR"font-exs-npc0110.txt", "fdb602-101010"},
};
...
void signlecpy_pri_InitCriticalSection()
{
    ...
    {
        api_log_MsgError("create signlecpy_pri_InitCriticalSection lock failed");
        exit(1);
    }
    return;
}
...
static int signle_checklifecpy1(Idmsoft* dmobj, void* ctxt, int recusive)
{
    ...
    dmobj->SetDict(0, FONTSIGNLECPY1_DIR"font-blood.txt");
    ...
    signle_checkkeystatecpy1(dmobj, ctxt);
    ...
    for (;;)
    {
        ...
        dmobj->SetDict(0, FONTSIGNLECPY1_DIR"font-chcklife.txt");
    ...
    for (;;)
    {
        code = signle_checklifecpy1(dmobj, ctxt, 1);
...
LABEL_retalk_with_NPC_in_cpy1:
    ...
    for (;;)
    {
        ...
        code = check_pri_copyfull(dmobj, ctxt);
        if (SNAIL_ERRNO_SUCCESS != code)
        {
...
static int fight_auto_stop(Idmsoft* dmobj, void* ctxt)
{
    dmobj->KeyDownChar("w");
    Delay(25);
    dmobj->KeyUpChar("w");
...
static int fight_auto_start(Idmsoft* dmobj, void* ctxt)
{
    struct clumantask* luaman_task = (struct clumantask*)ctxt;
    struct cluamantaskctxt* luaman_taskctxt = &luaman_task->ctxt;
    struct cluamanconfig* luaman_config = (struct cluamanconfig*)(luaman_taskctxt->context);
    ...
    for (;;)
    {
        ...
        dmobj->SetDict(0, FONTSIGNLECPY1_DIR"font-fightauto.txt");
...
static int fight_cpy4_pri_timeout_auto(Idmsoft* dmobj, void* ctxt, int withu,
    struct font_info_t* fontinfo, int timeout, char* adjpos)
{
    struct clumantask* luaman_task = (struct clumantask*)ctxt;
    struct cluamantaskctxt* luaman_taskctxt = &luaman_task->ctxt;
    struct cluamanconfig* luaman_config = (struct cluamanconfig*)(luaman_taskctxt->context);
    ...
    api_log_MsgDebug("副本4:自动:打怪:开始");

    code = fight_auto_start(dmobj, ctxt);
    if (SNAIL_ERRNO_REALIVE == code)
    {
        api_log_MsgDebug("自动:打怪:开始:血条:死亡,复活, adjpos:%s", adjpos);
        return code;
    }
    ...
    api_log_MsgDebug("副本4:自动:打怪:完成");
    return SNAIL_ERRNO_SUCCESS;
}


static int fight_cpy4_pri_timeout(Idmsoft* dmobj, void* ctxt, int withu,
    struct font_info_t* fontinfo, int timeout, char* adjpos)
{
    struct clumantask* luaman_task = (struct clumantask*)ctxt;
    struct cluamantaskctxt* luaman_taskctxt = &luaman_task->ctxt;
    ...
    for (;;)
    {
        ...
        if (NULL != adjpos)
        {
            pathOfPosXYs = CString(adjpos);
            code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
            if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
            {
                api_log_MsgDebug("打怪:归位:坐标:[%s]:死亡:复活", adjpos);
                return SNAIL_ERRNO_REALIVE;
            }

            api_log_MsgDebug("打怪:归位:坐标:[%s]:成功", adjpos);
        }
        api_log_MsgDebug("查找:失败");
        Delay(500);
    }
...
LABEL_find_failed_after_npc_talk:
    ...
    pathofposxys = "449:383|442:383|433:383|424:383|416:383|408:383|400:383";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, &fi_pri[0], 20, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "387:383|381:383|372:384|364:384|357:384|350:384|340:384|332:384|323:382|310:383";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, &fi_pri[0], 30, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "306:375|306:367|304:352|303:346|300:340|295:335|291:328|291:321|290:313|291:305|293:293";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, &fi_pri[1], 120, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "291:285|289:278|287:271|285:265|286:258|286:251|286:242|285:234|284:227";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    //code = beat_cpy_timeout(dmobj, ctxt, 10, 1);
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, NULL, 70, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "286:217|285:208|280:199|271:196|265:201|262:209";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    //code = beat_cpy_timeout(dmobj, ctxt, 10, 1);
    //code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, &fi_pri[0], 60, "262:209");
    code = fight_cpy4_pri_timeout_auto(dmobj, ctxt, 1, &fi_pri[0], 60, "262:209");
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "260:215|259:223|255:230|248:237|242:240|235:241|228:241|220:241|212:239|205:235|195:232|188:225";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    //code = beat_cpy_timeout(dmobj, ctxt, 10, 1);
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, NULL, 50, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "180:228|174:228|164:227|161:219|160:207|167:198|178:196";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, &fi_pri[0], 60, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "189:192|197:187|199:179|199:172|199:162|199:155|199:148";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    pathofposxys = "199:132|198:125|198:116|198:107";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
    ...
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 0, &fi_pri[2], 0, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    code = fight_cpy4_pri_timeout(dmobj, ctxt, 1, NULL, 10, NULL);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
        api_log_MsgDebug("打怪:超时:死亡:复活");
        return SNAIL_ERRNO_REALIVE;
    ...
    pathofposxys = "199:99|200:93|200:89";
    pathOfPosXYs = CString(pathofposxys);
    code = signle_walkcpy1(dmobj, ctxt, pathOfPosXYs, luaman_config->direction);
    if (SNAIL_ERRNO_REALIVE == code || SNAIL_ERRNO_BLOCKED == code)
    {
