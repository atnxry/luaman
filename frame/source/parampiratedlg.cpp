// ParamPirateDlg.cpp : implementation file
//
...
ParamPirateDlg::ParamPirateDlg(CWnd* pParent /*=NULL*/)
	: CDialog(ParamPirateDlg::IDD, pParent), CParameters()
{
	//{{AFX_DATA_INIT(ParamPirateDlg)
...
void ParamPirateDlg::SetDirection(int direction)
{
    m_config.direction = direction;
    return;
}

void ParamPirateDlg::SetRole(int role)
{
    m_config.role = (ROLE_E)role;
    return;
}

void ParamPirateDlg::SetGroup(int grp)
{
    m_config.groupid = grp;
    return;
}

void ParamPirateDlg::SetBlueMax(int bluemax)
{
    m_config.maxBlueFiled = bluemax;
    return;

}
void ParamPirateDlg::SetBlueMin(int bluemin)
{
    m_config.minBlue = bluemin;
    return;
}
void ParamPirateDlg::SetBlueThreshold(int bluethreshold)
{
    m_config.blueThreshold = bluethreshold;
    return;
}

void ParamPirateDlg::SetServer(char* server)
{
    memcpy(m_config.alarmServer, server, strlen(server) + 1);
    return;
}
void ParamPirateDlg::SetPort(int port)
{
    m_config.alarmPort = port;
    return;
}

void ParamPirateDlg::SetHost(int host)
{
    m_config.alarmHostIndex = host;
    return;
}

void ParamPirateDlg::SetLoopNum(unsigned int loopnum)
{
...
BOOL ParamPirateDlg::OnInitDialog()
{
    ...
    m_config.taskType = TASK_TYPE_PIRATE;
...
void ParamPirateDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(ParamPirateDlg)
...
BEGIN_MESSAGE_MAP(ParamPirateDlg, CDialog)
	//{{AFX_MSG_MAP(ParamPirateDlg)
    ON_BN_CLICKED(IDC_BUTTON_alarm_test, OnBUTTONalarmtest)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// ParamPirateDlg message handlers

void ParamPirateDlg::OnOK() 
{
...
void ParamPirateDlg::OnBUTTONalarmtest() 
{
