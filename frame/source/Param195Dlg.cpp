// Param195Dlg.cpp : implementation file
//
...
CParam195Dlg::CParam195Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CParam195Dlg::IDD, pParent), CParameters()
{
	//{{AFX_DATA_INIT(CParam195Dlg)
...
void CParam195Dlg::SetDirection(int direction)
{
    m_config.direction = direction;
    return;
}

void CParam195Dlg::SetRole(int role)
{
    m_config.role = (ROLE_E)role;
    return;
}

void CParam195Dlg::SetGroup(int grp)
{
    m_config.groupid = grp;
    return;
}

void CParam195Dlg::SetBlueMax(int bluemax)
{
    m_config.maxBlueFiled = bluemax;
    return;

}
void CParam195Dlg::SetBlueMin(int bluemin)
{
    m_config.minBlue = bluemin;
    return;
}
void CParam195Dlg::SetBlueThreshold(int bluethreshold)
{
    m_config.blueThreshold = bluethreshold;
    return;
}

void CParam195Dlg::SetServer(char* server)
{
    memcpy(m_config.alarmServer, server, strlen(server) + 1);
    return;
}
void CParam195Dlg::SetPort(int port)
{
    m_config.alarmPort = port;
    return;
}

void CParam195Dlg::SetHost(int host)
{
    m_config.alarmHostIndex = host;
    return;
}

void CParam195Dlg::SetLoopNum(unsigned int loopnum)
{
...
BOOL CParam195Dlg::OnInitDialog()
{
    ...
    m_config.taskType = TASK_TYPE_195;
...
void CParam195Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CParam195Dlg)
...
BEGIN_MESSAGE_MAP(CParam195Dlg, CDialog)
	//{{AFX_MSG_MAP(CParam195Dlg)
	ON_BN_CLICKED(IDC_BUTTON_alarm_test, OnBUTTONalarmtest)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CParam195Dlg message handlers
void CParam195Dlg::OnOK() 
{
    int j = 0;
    char* bluemax[] = BLUE_MAX_DESC_ARRAY();
    char* bluemin[] = BLUE_MIN_DESC_ARRAY();
...
void CParam195Dlg::OnBUTTONalarmtest() 
{
