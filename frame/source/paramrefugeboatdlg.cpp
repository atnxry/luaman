// ParamRefugeboatDlg.cpp : implementation file
//
...
void CParamRefugeboatDlg::SetDirection(int direction)
{
    m_config.direction = direction;
    return;
}

void CParamRefugeboatDlg::SetRole(int role)
{
    m_config.role = (ROLE_E)role;
    return;
}

void CParamRefugeboatDlg::SetGroup(int grp)
{
    m_config.groupid = grp;
    return;
}

void CParamRefugeboatDlg::SetBlueMax(int bluemax)
{
    m_config.maxBlueFiled = bluemax;
    return;

}
void CParamRefugeboatDlg::SetBlueMin(int bluemin)
{
    m_config.minBlue = bluemin;
    return;
}
void CParamRefugeboatDlg::SetBlueThreshold(int bluethreshold)
{
    m_config.blueThreshold = bluethreshold;
    return;
}

void CParamRefugeboatDlg::SetServer(char* server)
{
    memcpy(m_config.alarmServer, server, strlen(server) + 1);
    return;
}
void CParamRefugeboatDlg::SetPort(int port)
{
    m_config.alarmPort = port;
    return;
}

void CParamRefugeboatDlg::SetHost(int host)
{
    m_config.alarmHostIndex = host;
    return;
}

void CParamRefugeboatDlg::SetLoopNum(unsigned int loopnum)
{
    m_config.loop_num = loopnum;
    return;
}

void CParamRefugeboatDlg::SetPalyWndIndex(unsigned int playWndIndexVal)
{
    m_config.playWndIndexVal = playWndIndexVal;
    return;
}

void CParamRefugeboatDlg::SetTransWait(int Wait2Sleep)
{
    m_config.Wait2Sleep = Wait2Sleep;
    return;
}

void CParamRefugeboatDlg::SetExitGameCopyFull(int exitGameCopyFull)
{
    m_config.exitGameCopyFull = exitGameCopyFull;
    return;
}

void CParamRefugeboatDlg::SetCallBabyDie(int callbabydie)
{
    m_config.callbabydie = callbabydie;
    return;
}

void CParamRefugeboatDlg::SetCleanPackageTime(int cleanPackageTime)
{
    m_config.cleanPackageTime = cleanPackageTime;
}

void CParamRefugeboatDlg::SetCleanPackageWhenStart(int cleanPackageWhenStart)
{
    m_config.cleanPackageWhenStart = cleanPackageWhenStart;
    return;
}

void CParamRefugeboatDlg::SetCleanPackage1(int cleanPackage1)
{
    if (cleanPackage1)
    {
...
void CParamRefugeboatDlg::SetCleanPackage2(int cleanPackage2)
{
    if (cleanPackage2)
    {
...
void CParamRefugeboatDlg::SetCleanPackage3(int cleanPackage3)
{
    if (cleanPackage3)
    {
...
void CParamRefugeboatDlg::SetCleanPackage4(int cleanPackage4)
{
    if (cleanPackage4)
    {
...
void CParamRefugeboatDlg::SetCleanPackage5(int cleanPackage5)
{
    if (cleanPackage5)
    {
...
void CParamRefugeboatDlg::SetCleanPackage(int cleanPackage)
{
    m_config.cleanPackageX = cleanPackage;
}

void CParamRefugeboatDlg::SetWaitCopytime(int waitCopytime)
{
    m_config.waitCopytime = waitCopytime;
}

void CParamRefugeboatDlg::SetBluebloodChecktime(int bluebloodChecktime)
{
    m_config.bluebloodChecktime = bluebloodChecktime;
}

void CParamRefugeboatDlg::SetBlockTimes(int blockTimes)
{
    m_config.blockTimes = blockTimes;
}
#endif

BOOL CParamRefugeboatDlg::OnInitDialog()
{
    ...
    m_config.taskType = TASK_TYPE_REFUGEBOAT;
    ...
    //新增
    //清包时间,小时
    char* cleanPackageTime[] = {"2", "3", "4", "6", "8", "10", "12", "120"};
...
CParamRefugeboatDlg::CParamRefugeboatDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CParamRefugeboatDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CParamRefugeboatDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CParamRefugeboatDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CParamRefugeboatDlg)
...
BEGIN_MESSAGE_MAP(CParamRefugeboatDlg, CDialog)
	//{{AFX_MSG_MAP(CParamRefugeboatDlg)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CParamRefugeboatDlg message handlers

void CParamRefugeboatDlg::OnOK() 
{
...
void CParamRefugeboatDlg::OnCancel() 
{
