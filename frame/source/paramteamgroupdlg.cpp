// ParamTeamGroupDlg.cpp : implementation file
//
...
void CParamTeamGroupDlg::SetSelectedFrontIndex(int selected_front_index)
{
    m_config.selected_front_index = selected_front_index;
    return;
}

void CParamTeamGroupDlg::SetDirection(int direction)
{
    m_config.direction = direction;
    return;
}

void CParamTeamGroupDlg::SetRole(int role)
{
    m_config.role = (ROLE_E)role;
    return;
}

void CParamTeamGroupDlg::SetGroup(int grp)
{
    m_config.groupid = grp;
    return;
}

void CParamTeamGroupDlg::SetBlueMax(int bluemax)
{
    m_config.maxBlueFiled = bluemax;
    return;

}
void CParamTeamGroupDlg::SetBlueMin(int bluemin)
{
    m_config.minBlue = bluemin;
    return;
}
void CParamTeamGroupDlg::SetBlueThreshold(int bluethreshold)
{
    m_config.blueThreshold = bluethreshold;
    return;
}

void CParamTeamGroupDlg::SetServer(char* server)
{
    memcpy(m_config.alarmServer, server, strlen(server) + 1);
    return;
}
void CParamTeamGroupDlg::SetPort(int port)
{
    m_config.alarmPort = port;
    return;
}

void CParamTeamGroupDlg::SetHost(int host)
{
    m_config.alarmHostIndex = host;
    return;
}

void CParamTeamGroupDlg::SetLoopNum(unsigned int loopnum)
{
    m_config.loop_num = loopnum;
    return;
}

void CParamTeamGroupDlg::SetPalyWndIndex(unsigned int playWndIndexVal)
{
    m_config.playWndIndexVal = playWndIndexVal;
    return;
}

void CParamTeamGroupDlg::SetTransWait(int Wait2Sleep)
{
    m_config.Wait2Sleep = Wait2Sleep;
    return;
}

void CParamTeamGroupDlg::SetExitGameCopyFull(int exitGameCopyFull)
{
    m_config.exitGameCopyFull = exitGameCopyFull;
    return;
}

void CParamTeamGroupDlg::SetCleanPackageWhenStart(int cleanPackageWhenStart)
{
...
BOOL CParamTeamGroupDlg::OnInitDialog()
{
    char* selected_font[] = SELECTED_FONT();
    ...
    m_config.taskType = TASK_TYPE_TEAMGROUP;

    //api_log_MsgDebug("CParamTeamGroupDlg::OnInitDialog");
    //api_PrintConfig(&m_config);

    m_combo_selected_front_index.Clear();
    for (j = 0; j < sizeof(selected_font)/sizeof(selected_font[0]); ++j)
    {
        m_combo_selected_front_index.InsertString(j, selected_font[j]);
    }
    ...
    m_combo_selected_front_index.SetCurSel(m_config.selected_front_index);
...
CParamTeamGroupDlg::CParamTeamGroupDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CParamTeamGroupDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CParamTeamGroupDlg)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void CParamTeamGroupDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CParamTeamGroupDlg)
	...
	DDX_Control(pDX, IDC_COMBO_selected_font_index, m_combo_selected_front_index);
...
BEGIN_MESSAGE_MAP(CParamTeamGroupDlg, CDialog)
	//{{AFX_MSG_MAP(CParamTeamGroupDlg)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CParamTeamGroupDlg message handlers

void CParamTeamGroupDlg::OnOK() 
{
	...
	// TODO: Add extra validation here
    m_config.selected_front_index = m_combo_selected_front_index.GetCurSel();
...
void CParamTeamGroupDlg::OnCancel() 
{
